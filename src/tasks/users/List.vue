...
<script setup> 
    ...
    const setColumns=()=>{
      ...
      columns[key]={
        ...
        filterable:true,
        type:'dropdown',
        filter:{from:'',to:'',options:taskData.user_groups.map((item)=>{return{label:item.name,value:item.name}})}
      };
      key='email';
      columns[key]={
        label: labels.get('label_'+key),
    ...
    //     if(item_id>0){
    //         router.push(taskData.api_url+'/edit/'+item_id);
    //     }
    // }
    // const detailsDirect=()=>{
    //     let item_id=$('#input_item_id_edit').val();
    //     if(item_id>0){
    //        router.push(taskData.api_url+'/details/'+item_id);
    //     }
    // }
</script>
